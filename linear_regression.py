...
def main():
    ...
    if data is not None:
        preprocessor = Preprocessor(data, ['trip_distance(km)'], ['fuel_note','manufacturer', 'model', 'version', 'odometer',
           'trip_distance(km)', 'fuel_type','tire_type', 'driving_style'])
        X, Y, df_clean = preprocessor.preprocess()
    ...
    if data_enc_l is not None:
        preprocessor = Preprocessor(data_enc_l, ['trip_distance(km)'], ['fuel_note','manufacturer', 'model', 'version', 'odometer',
           'trip_distance(km)', 'fuel_type','tire_type', 'driving_style'])
        X_enc_l, Y_enc_l, df_clean_enc_l = preprocessor.preprocess()
    ...
    if data_enc_dum is not None:
        preprocessor = Preprocessor(data_enc_dum, ['trip_distance(km)'], ['fuel_note','manufacturer', 'model', 'version', 'odometer',
           'trip_distance(km)', 'fuel_type'])
        X_enc_dum, Y_enc_dum, df_clean_enc_dum = preprocessor.preprocess()
